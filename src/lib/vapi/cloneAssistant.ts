...
export const VOIZO_SYSTEM_PREFIX = [
  ...
  ``,
  `1. TEXT MESSAGE HANDLING: When your script calls for sending the customer a text message,`,
  `   verbally confirm by saying something like "I'll send you a text message now" before`,
  `   moving on. Always say "text message", never "SMS" — the voice spells SMS letter by`,
  `   letter. If the customer objects to being texted or asks not to be`,
  `   contacted, do NOT promise or send a text message — accept that answer the FIRST`,
  `   time, acknowledge politely, and move on. Never pressure the customer about`,
  `   the text message. If the agent script below conflicts with this rule, follow THIS rule.`,
  ``,
  `2. CALL ENDING: Never end the call immediately after the customer agrees to receive`,
  `   a text message. Confirm the text message is on its way first, then wrap up the call politely.`,
  ``,
