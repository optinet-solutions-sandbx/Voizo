...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [numbers, calls, sms] = await Promise.all([
    ...
    fetchAllRows(supabaseAdmin, "calls_v2", "campaign_id, campaign_number_id, status, goal_reached, duration_seconds, created_at, voicemail, ended_reason, transcript", "id", eq),
