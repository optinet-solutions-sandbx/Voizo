...
-- `call_delay_minutes` = how long after a sign-up FIRST APPEARS in the segment
-- before we dial (null = right away). App enforces the 1440 (24h) ceiling.
alter table public.campaigns_v2
  add column if not exists call_delay_minutes integer
  check (call_delay_minutes is null or call_delay_minutes > 0);
...
--    'waiting' = admitted (phone validated) but dial row deferred until
--    first_seen_at + parent.call_delay_minutes (operator call delay).
create table if not exists public.realtime_seen_members (
  ...
  status text not null check (status in
    ('queued','waiting','rejected_country','no_phone','invalid_phone','lookup_failed')),
