...
async function main() {
  ...
  // Created for their side effect only — the edges reference these by intent_key
  // string, and their descriptions feed the classifier vocabulary.
  ...
  await mkMatcher("valr_int_follow_up", "Player asks a follow-up question or wants to know more about the offer");
  await mkMatcher("valr_int_wrapping_up", "Player signals the call is wrapping up or ending (thanks, sounds good, got to go, bye)");
  await mkMatcher("valr_int_refuse_sms", "Player explicitly refuses the SMS or says do not text me");
  await mkMatcher("valr_int_do_not_call", "Player asks not to be called again or to be put on the do-not-call list");
